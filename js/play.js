...
function dealCards() {
  stacks.forEach((stack, idx) => {
    for (let i = 0; i < idx + 1; i++) stack.unshift(deck.shift());
  });
  deck.forEach((card) => {
    pile.push(card);
  });
...
function handleClick(evt) {
  ...
  if (clickDest.includes('stack')) {
    isDoubleClick()
      ? handleStackDoubleClick(evt.target)
...
function handleStackDoubleClick(element) {
  ...
  if (document.querySelector('.highlight')) {
    ...
    if (
      isTheSameCard(highlightEl, clickedCard) &&
      clickDest === getClickDestination(highlightEl)
    ) {
...
function handleAceClick(element) {
  ...
  if (!clickedCard && isFaceUpCard(element)) {
    ...
    if (!topCard) {
      ...
      if (
        getCardValue(clickedCard) === getCardValue(topCard) + 1 &&
